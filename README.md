...
v1.2 全新设计：每个开关都有自己的颜色，开启时变成发光的渐变色块；面板顶部可以一键切换 🎬 演示 / 🎧 专注 / 🌙 夜间 **场景**，还有 🍅 **专注计时**。面板背景在 macOS 26 / 27 上是「液态玻璃」，在 macOS 13–15 上自动换成毛玻璃。
...
| | SwitchBar 的做法 |
|---|---|
| 源码 | 全部开源，约 4800 行 Swift，自己审查、自己编译 |
...
面板顶部的三个胶囊按钮，点一下就把一组开关全部打开，再点一下恢复原样（原本就开着的开关不会被关掉）：
...
点面板顶部的 🍅 开始倒计时（默认 25 分钟），菜单栏会显示剩余时间；默认会自动开启「专注」场景，时间到了播放提示音、弹出提醒，并恢复原来的设置。在 🍅 上点右键可以选 15 / 25 / 45 / 60 分钟，或者再加 5 分钟。

### 电量

MacBook 上，面板右上角会显示电池电量，充电时变成绿色，低于 20% 时变成红色。
...
- **面板大小**：面板约 370 × 610 点，在 14 英寸屏幕所有缩放档位（包括「更大字体」1024 × 665）下都能完整显示；如果把开关全打开还嫌挤，可以在设置里隐藏不常用的开关。
